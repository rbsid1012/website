...
{hideFullHeader ? (
  <div className="flex justify-end">
    <motion.nav className="flex items-center justify-end px-4 py-2.5 rounded-full border border-white/10">
      <motion.button
        onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
        className="p-3 text-foreground rounded-full"
      >
  ...
  <div className="lg:hidden">
    ...
    >
      <div className="p-4 space-y-1 rounded-2xl bg-black/95 border border-white/10">
        {navLinks.map((link) => (
          <Link
            key={link.label}
            to={link.href}
            onClick={() => handleNavClick(link.href)}
            className="flex items-center text-sm py-3 px-4 rounded-xl hover:bg-white/[0.06]"
          >
            {link.label}
          </Link>
        ))}
